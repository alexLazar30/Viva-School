...
# 4. On the line below we will select all the children with total absences lower than 20
...
# 6. On the line below we will select all the children with lastname Mihai
select * from children where last_name='Mihai';

# 7. On the line below we will select all the children whose year of birth is '2015'
select * from children where date_of_birth like '2015%';

# 8. On the line below we will select all the girls name from the class whose year of birth is '2012'
...
# 10. On the line below we will select all the children last_name with age of 10 and 13
select last_name, age from children where age=10 or age=13;

# 11. On the line below we will select children who have a number of absences of 30 and 35
...
# 14. On the line below we will calculate the average value for the total absences
...
# 16. On the line below we will calculate the number of girls from the school
select count(gender) as no_of_girls from children where gender='F';

# 17. On the line below we will calculate the total enrolled from the mandatory disciplines
select SUM(enrolled) as total_enrolled from mandatory_disciplines;

# 18. On the line below we will calculate the total enrolled from the optional disciplines
select SUM(enrolled) as total_enrolled from optional_disciplines;

# 19. On the line below we will calculate the child with the higher number of absences, using an aggregate function in a subquery
 select * from child_situation where total_absences=( select MAX(total_absences) from child_situation );
  
 # 20. On the line below, we will calculate the school curriculum with the lowest number of enrolled students by using an aggregate function in a subquery
...
# 22. On the line below we will get the fullname of the child and the total abscences using Inner Join statement between 'children' and 'child_situation' tables
